    ...
    return (
        <ScrollView className="flex-1 w-full bg-gray-100  ">
            ...
            <View className="w-full h-[60vh]  overflow-hidden">
                ...
                <View className="absolute  left-4 right-4 z-10 flex-row items-center p-4 shadow-md">
                    <TouchableOpacity className="p-2 border-2 border-gray-200 rounded-full">
                ...
                <Animated.View entering={LightSpeedInLeft} className="bg-white shadow-md rounded-2xl p-4 mt-6">
                    ...
                    <View className="flex-row justify-around">
                        {rideOptions.map((ride) => (
                            <TouchableOpacity
                                key={ride.id}
                                className={`items-center p-3 rounded-xl w-24 ${selectedRide === ride.id ? "border-2 border-blue-600 bg-gray-100" : "bg-white"
                                    }`}
                                onPress={() => setSelectedRide(ride.id)}
                            >
                                <Image source={ride.icon} className="w-12 h-12 mb-2" />
            ...
            <TouchableOpacity className="bg-white shadow-md rounded-2xl p-4 mt-4 flex-row items-center">
                <Image source={require("../assets/icon/cash.png")} className="w-8 h-8" />
                <Text className="ml-3 text-black text-lg font-semibold">Cash</Text>
                <Image source={require("../assets/icon/rightnav.png")} className="w-6 h-6 ml-auto" />
            </TouchableOpacity>

            {/* Next Button */}
            <TouchableOpacity
                className={`w-full py-4 rounded-[20px] mt-6 ${selectedRide ? "bg-blue-800" : "bg-gray-400"
                    }`}
